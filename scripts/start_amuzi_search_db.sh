...
echo "select concat(lower(artist.name), 'A', lower(album.name)) from artist join album on artist.id = album.artist_id into outfile '/tmp/artist_album_db.txt'" | mysql -u youbetter -pyoubetter youbetter
